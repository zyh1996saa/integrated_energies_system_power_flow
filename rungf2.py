...
def func(z):
    ...
    dp = np.array([((p[int(branches[i,0]-1)])**2-(p[int(branches[i,1]-1)])**2) for i in range(numofbran) ]).reshape(numofbran,)
    #print(dp)
    s = np.zeros((numofbran,))
    for i in range(numofbran):
        if p[int(branches[i,0])-1]**2>=p[int(branches[i,1])-1]**2:
            s[i] = 1
        else:
            s[i] = -1
    #print(p)
...
def rungf(case):
    ...
    clist = np.array([case['branch'][i,2] for i in range(numofbran)]).reshape(numofbran,1)
    plist = buses[:,_p]*1e6
    flist = buses[:,_f]
    ...
    print('func error in gas system:',func(res).sum())
    #print(res)
